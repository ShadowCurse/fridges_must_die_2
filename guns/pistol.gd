...
func _process(delta: float) -> void:
    if self.progress < 1.0:
      ...
      self.target_rotation.x = self.recoil_rotation_x.sample(1.0)
      self.target_rotation.z = self.recoil_rotation_z.sample(1.0)
      self.target_position.z = self.recoil_position_z.sample(1.0)
